...
#     if (all(ans$Scale == 1)) ans$Scale <- NULL
#
#     attributes(ans) <- list(class = "pIRList", fullList = ans.lst,
#                             group.names = groups, group.var = attr(ans.lst,"group.var"),scale=scale,call=call)
#
#     ans
#
#   } else {
...
#                         Scale = scale) # really to match Hmisc's binconf()
#       if(scale == 1) ans$Scale <- NULL
#       ans <- structure(ans,class="pIR",fullList = ans.lst,call=call,group.names = "", group.var = "", scale=scale)
...
#     if (all(ans$Scale == 1)) ans$Scale <- NULL
#
#     ans <- structure(ans, class = "pIRList", fullList = ans.lst,
#                      x.var = vars$x, m.var = vars$m, n.var = vars$n,
#                      group.names = groups, group.var = attr(ans.lst,"group.var"),scale=scale,call=call)
#
#
...
#     if(scale == 1) ans$Scale <- NULL
#
#     ans <- structure(ans, class = "pIR", fullList = ans.lst, group.names = names(ans),
...
#
# "[.pIR" <- get("[.data.frame")
# "[.pIRList" <- get("[.data.frame")
#
# # "as.data.frame.pIR" <- function(x, row.names = NULL, optional = FALSE,  ...){
# #   args <- list(...)
# #   #if(is.null(args$digits)) digits <- 4
# #   #else digits <- args$digits
# #   scale <- x$scale
...
# # }
#
# # "as.data.frame.pIRList" <- function(x, row.names = NULL, optional = FALSE, ...){
# #   n <- length(x)
# #   out <- data.frame(Group = attr(x,"group.names"),
# #                     PointEst = rep(0,n),
# #                     Lower = rep(0,n),
...
#            "mir" = x$CIEstName <- "Minimum Infection Rate"
#     )
#     structure(x,class="summary.pIR",call=attr(x,"call"),scale=x$scale)
#   }
#
# "print.summary.pIR" <-
#   function(x, ...){
...
#   out <- lapply(attr(object,"fullList"), sumf)
#   #attributes(out) <- list(class = "summary.pooledBinList", names = attr(object,"names"),group.var = attr(object,"group.var"))
#   attributes(out) <- list(class = "summary.pIRList",
#                           names = grp.names,group.var = attr(object,"group.var"),
#                           call= attr(object,"call"))
#   out
# }
...
#
#
# "[.summary.pIR" <- get("[.data.frame")
# "[.summary.pIRList" <- get("[.data.frame")
#
#
# "plot.pIR" <-
#   function(x,pch=16,refline=TRUE,printR2=TRUE,...){
...
#     x.lst <- attr(x,"fullList")
#     if(all(x.lst$n==1)) {
#       xmn <- as.list(by(x.lst$x,x.lst$m,function(x) c(sum(x),length(x))))
#       m <- as.numeric(names(xmn))
#       xx <- sapply(xmn,function(x) x[1])
#       n <- sapply(xmn,function(x) x[2])
#     } else {
