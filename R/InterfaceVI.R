...
#   ans <- data.frame(Group = groups,
#                     VI = rep(0,nGroups))
#   for(i in 1:nGroups) ans$VI[i] <- ans[[]]
#
#   ans <- structure(ans,class = "VIList", fullList = ans.lst, group.names = groups, group.var = group.var,
#                    vector = Vectors, vectors.var = vectors.var, traptime.var = traptime.var, #deparse(substitute(vectors)),
#                    p = ans.p, n = ans.nbar,
#                    n.use.na = n.use.na, n.use.traps = n.use.traps,
#                    pt.method = pt.method, ci.method = ci.method,
...
#
#   ans <- structure(ans,class = "VI", fullList = ans.lst, group.names = groups, group.var = group.var,
#                    vector = Vectors, vectors.var = vars$vector, traptime.var = traptime.var, #deparse(substitute(vectors)),
#                    p = ans.p, n = ans.nbar,
#                    n.use.na = n.use.na, n.use.traps = n.use.traps,
#                    pt.method = pt.method, ci.method = ci.method,
#                    call=call)
...
"summary.vectorIndex" <- function(object, ...){
  ...
  structure(list(vi=x$VI,vector=attr(x,"vectors"),
                 p=attr(x,"p"),n=attr(x,"n"),nGroups = attr(x,"n.groups"), nVectors =length(attr(x,"vector")),
                 vi.obj=object), class="summary.vectorIndex")
...
"summary.VI" <- function(object, ...){
  ...
  structure(list(vi=x$VI,vector=attr(x,"vectors"),
                 p=attr(x,"p"),n=attr(x,"n"),nGroups = attr(x,"n.groups"), nVectors =length(attr(x,"vector")),
                 vi.obj=object), class="summary.VI")
