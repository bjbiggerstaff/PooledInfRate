...
#"[.pIRDiff" <- subsetDF #get("[.data.frame")
#"[.summary.pIRDiff" <- subsetDF #get("[.data.frame")

"[.pIRDiff" <-  function (x, i, j, drop = if (missing(i)) TRUE else length(cols) ==  1)  {
  mdrop <- missing(drop)
  Narg <- nargs() - !mdrop
  has.j <- !missing(j)
...
"[.summary.pIRDiff" <-  function (x, i, j, drop = if (missing(i)) TRUE else length(cols) ==  1)  {
  mdrop <- missing(drop)
  Narg <- nargs() - !mdrop
  has.j <- !missing(j)
  if (!all(names(sys.call()) %in% c("", "drop")) &&
...
"summary.pIRDiff" <- function(object, simple = FALSE, ...){
  ...
  #cat("\nCall: ", deparse(x$call), "\n\n")
  structure(object, class="summary.pIRDiff", df=object, grp.pooledBin = grp.pb, PtEstName = PtEstName, CIEstName = CIEstName,
...
#
#
# "print.pIRDiff" <- function(x, ...,digits=NULL,quote=FALSE,right=TRUE,row.names=TRUE,max=NULL){
#   print(as.data.frame(unclass(x)),...,digits=digits,quote=quote,right=right,row.names=row.names,max=max)
#   invisible(x)
# }
#
# "[.pIRDiff" <- get("[.data.frame")
#
...
# # }
#
# "summary.pIRDiff" <- function(object, scale=attr(x,"scale"), ...){
#     x <- attr(object,"fullList")
#     args <- list(...)
#     if(is.null(args$digits)) digits <- 4
#     else digits <- args$digits
...
#     #summat <- matrix(c(scale*p1$p, scale*p1$lcl, scale*p1$ucl, scale, sum(x$n1 * x$m1), sum(x$n1), sum(x$x1),
#     #                   scale*p2$p, scale*p2$lcl, scale*p2$ucl, scale, sum(x$n2 * x$m2), sum(x$n2), sum(x$x2)), nrow=2,ncol=7, byrow=TRUE)
#     grp.pooledBin <- attr(x, "grp.pooledBin")
#     grp.pb <- t(sapply(grp.pooledBin,
#                        function(x) c(x$scale*x$p, x$scale*x$lcl, x$scale*x$ucl, x$scale, sum(x$n * x$m), sum(x$n), sum(x$x))))
#     #summat <- round(summat, digits=digits)
#     #dimnames(grp.pb) <- list(attr(x,"group.names"), c("PointEst","Lower","Upper","Scale","Individuals","Pools","Positive Pools"))
#     dimnames(grp.pb) <- list(rep("",nrow(grp.pb)), c("P","Lower","Upper","Scale","Individuals","Pools","Positive Pools"))
#     grp.pb <- as.data.frame(grp.pb)
#     grp.pb$Group <- attr(x,"group.names")
#     grp.pb <- grp.pb[,c(8,1:7)]
#     names(grp.pb)[1] <- attr(x,"group.var")
#     rownames(grp.pb) <- 1:nrow(grp.pb)
#     #cat("\nCall: ", deparse(x$call), "\n\n")
#     structure(grp.pb, class="summary.pIRDiff",
#               grp.pooledBin = grp.pb, PtEstName = PtEstName, CIEstName = CIEstName)
#   }
#
#
#
