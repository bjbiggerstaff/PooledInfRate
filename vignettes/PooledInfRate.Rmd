...
and
```{r}
ex2.dat <- data.frame(Pos = c(2,1,0,1), PoolSize = c(50,25,10,5), NumPools = c(10,10,5,2), Location = c("A","A","B","B"))
ex2.dat
```
and if there are different groups, say <tt>Species</tt>, 
```{r}
mosq.dat <- data.frame(Zone = c(1,1,2,2, 1,1,1,2,2, 1,1,2),
                       Species = rep(c("Culex pipiens","Culex tarsalis", "Culex quinquefaciatus"), c(4,5,3)),
...
pIR(Pos ~ PoolSize, mosq.dat, pt.method = "mir", ci.method = "mir") # specification of "mir" for either sets the other to "mir"
```
...
mdiff.out <- pIRDiff(Pos ~ PoolSize | Species, mosq.dat)
summary(mdiff.out)
```
#### Missing data
...
```{r}
vi.out <- with(mosq.dat, vectorIndex(Pos,PoolSize,vector=Species)) # or VI(Pos,PoolSize,vector=Species)
vi.out
```
...
```{r}
pools.dat <- data.frame(Species=c("Cx. tarsalis","Cx. pipiens"),Pos=c(1,1),PoolSize=c(50,50),NumPools=c(6,5))
pools.dat
```
...
```{r}
traps.dat <- data.frame(Species = c("Cx. tarsalis","Cx. pipiens"),Pos = c(NA,NA),PoolSize=c(442, 233),NumPools=c(1,1)) # note NumPools should be 1 for each entry
...
```{r}
vi.dat$TrapNights <- c(1,1,6,6) # put 1s in for the pool data for later; use values for trap nights in application
vi.dat

VI(Pos ~ PoolSize + n(NumPools) / Species:TrapNights, vi.dat, n.use.traps = FALSE, n.use.na = TRUE)
summary(VI(Pos ~ PoolSize + n(NumPools) / Species:TrapNights, vi.dat, n.use.traps = FALSE, n.use.na = TRUE))
```
...
VI(Pos ~ PoolSize + n(NumPools) / Species:TrapNights, vi.dat, n.use.traps = TRUE, n.use.na = TRUE)
summary(VI(Pos ~ PoolSize + n(NumPools) / Species:TrapNights, vi.dat, n.use.traps = TRUE, n.use.na = TRUE))
```
